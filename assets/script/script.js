...
for (const plat of platsAfricain) {
    ...
    prixPlat.textContent = plat.prix + " FCFA"
    ...
    ButtonPlat.onclick = function (e) {
        ...
        td4.textContent = produit.prix+" FCFA";
        td4.setAttribute("id","produit-prix-" + produit.id);
        tr.appendChild(td4);

        let td5 = document.createElement("td");
        td5.textContent = produit.prix+ " FCFA";
...
function AjouterProduit(id){
    ...
    prixProduitT.textContent = prixproduitTotal+" FCFA"
...
function SupprimerProduit(id){
    let qteProduit = document.getElementById(`quantite-total-${id}`);
    qteProduit.textContent = 0;
    let prixProduitT = document.getElementById(`produit-total-${id}`);
    prixProduitT.textContent = 0;
    let prixProduitU = document.getElementById(`produit-prix-${id}`);
    prixProduitU.textContent = 0;
    let tr = document.getElementById(`bouton-sup-${id}`).closest("tr");
    tr.remove();
